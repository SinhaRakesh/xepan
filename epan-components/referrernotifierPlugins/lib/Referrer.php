	...
	function ExtractKeywords( $refurl ) {
		...
		$ses=array(
			...
			'Google WAP'=>array( 'url'=>'wap.google.com', 'query'=>'q',
				'query2'=>'query' ),
			'Google (India)'=>array( 'url'=>'google.co.in', 'query'=>'q' ),
			...
			foreach ( $ses as $name=>$seng ) {
				...
				if ( ereg( $seng['url']."/", $refhost ) ) {
					if ( isset( $seng['home'] ) ) {
						...
						$result['searchengineurl']="http://".$seng['home']."/";
					} else {
						// other wise 'build' the SE's URL based on the search URL
						$result['searchengineurl']="http://www.".$seng['url']."/";
					}
					...
					if ( preg_match( '/'.$seng['query'].'=(.*)(?:&|$)/Ui', $refquery, $regs ) ) {
						...
						$keywords=$searched;
						$result['keywords']=split( ' ', $searched ); // split up the keywords
						$result['searchengine']=$name; // store the name of the search engine
					} else {
